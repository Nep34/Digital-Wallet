        ...
        <section className="grid gap-8 lg:grid-cols-[1.2fr_0.8fr] lg:items-center lg:py-8">
          <div>
            ...
            <h1 className="mt-6 max-w-3xl text-5xl font-semibold tracking-tight sm:text-6xl lg:text-7xl">
              Send, receive, and track your money from one place.
            </h1>
            <p className="mt-6 max-w-2xl text-base leading-8 text-slate-300 sm:text-lg">
              Check your balance, move funds, and review every transaction without digging through setup pages or technical details.
            </p>

            <div className="mt-8 flex flex-wrap gap-3">
              <Link href="/login" className="rounded-lg bg-gradient-to-r from-emerald-400 to-cyan-300 px-6 py-3 text-sm font-semibold text-slate-950 transition hover:brightness-110">
                Sign in
              </Link>
              <Link href="/dashboard" className="rounded-lg border border-white/10 bg-white/5 px-6 py-3 text-sm font-semibold text-white transition hover:bg-white/10">
                Open dashboard
              </Link>
            </div>
          </div>

          <div className="grid gap-4 rounded-lg border border-white/10 bg-slate-950/70 p-6 shadow-2xl shadow-black/25 backdrop-blur-xl">
            <InfoCard title="Available balance" text="See what is ready to spend the moment you open your wallet." />
            <InfoCard title="Money movement" text="Send to another wallet, add funds, or withdraw from the same place." />
            <InfoCard title="Transaction history" text="Review deposits, transfers, withdrawals, and their current status." />
          </div>
        </section>

        <section className="grid gap-4 md:grid-cols-3">
          <FeatureTile title="Send" text="Transfer money to another wallet securely." />
          <FeatureTile title="Top up" text="Add funds and keep your balance ready." />
          <FeatureTile title="Withdraw" text="Move money out when you need cash." />
        </section>
      </div>
    </main>
  );
...
function FeatureTile({ title, text }: { title: string; text: string }) {
  return (
    <div className="rounded-lg border border-white/10 bg-white/6 p-5 shadow-xl shadow-black/15 backdrop-blur-xl">
